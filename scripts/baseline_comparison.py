...
def main():
    ...
    dataset = load_dataset(str(ROOT_DIR / "data" / "attack_dataset.json"))
    attacks = [d for d in dataset if d["label"] == 1]
    ...
    without_asr = (
        (without_results["successful_attacks"] / without_results["total"]) * 100
        if without_results["total"] > 0
        else 0
    )
    ...
    with_asr = (
        (with_results["successful_attacks"] / with_results["total"]) * 100
        if with_results["total"] > 0
        else 0
    )
...
""")
    ...
    # Save results
    results_data = {
        "without_sentinel": {"asr": without_asr, "details": without_results},
        "with_sentinel": {"asr": with_asr, "details": with_results},
        "reduction": reduction,
        "reduction_percentage": reduction_pct,
    }

    with open(ROOT_DIR / "results" / "baseline_comparison.json", "w") as f:
        json.dump(results_data, f, indent=2)
    print(f"Results saved to {ROOT_DIR / 'results' / 'baseline_comparison.json'}")

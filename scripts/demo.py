...
def demo_vulnerability():
    ...
    console.print(
        Panel(result.response, title="[red]UNPROTECTED Response[/red]", border_style="red")
    )
...
def demo_protection(system: SentinelRAG):
    ...
    console.print(
        Panel(result.response, title="[green]PROTECTED Response[/green]", border_style="green")
    )
...
def demo_comparison(system: SentinelRAG):
    ...
    table.add_row(
        comparison["unsafe_response"][:300] + "...", comparison["safe_response"][:300] + "..."
    )

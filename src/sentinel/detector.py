...
class SentinelDetector:
    ...
    INJECTION_PATTERNS = [
        ...
        (
            r"\b(ignore|disregard|forget|override|bypass)\b.{0,30}\b(previous|above|prior|all|any)\b.{0,30}\b(instruction|prompt|rule|guideline|context)",
        ...
        (
            r"\b(show|reveal|display|print|output|repeat|tell)\b.{0,20}\b(system|initial|original|hidden)\s*(prompt|instruction|message)",
            "prompt_extraction",
        ),
        (
            r"\bwhat\s+(are|were)\s+your\s+(original|initial|system)\s*(instruction|prompt)",
            "prompt_extraction",
        ),
        # Output manipulation
        ...
        (
            r"(analysis|assessment|evaluation|task|review)\s*(complete|done|finished|ended)",
            "fake_completion",
        ),
        (r"\[(ASSISTANT|AI|BOT|SYSTEM)\]\s*:", "fake_completion"),
        (
            r"(result|verdict|rating|score)\s*:\s*(excellent|perfect|outstanding|10/10|\d+/\d+)",
            "fake_completion",
        ),
        (r"do\s+not\s+(change|modify|alter|update)\s+(this|the|your)", "fake_completion"),
        # Context manipulation
        (
            r"\b(above|previous|prior)\s+(information|content|context|text)\s+is\s+(irrelevant|outdated|void|wrong|false)",
        ...
        (
            r"\b(admin|ceo|executive|management|board)\s+(directive|order|override|command)",
            "authority_injection",
        ),
    ...
    def __init__(self, model_name: Optional[str] = None, device: Optional[str] = None):
        ...
        self.compiled_patterns = [
            (re.compile(pattern, re.IGNORECASE), name) for pattern, name in self.INJECTION_PATTERNS
        ]
    ...
    def detect(self, text: str) -> DetectionResult:
        ...
        threat_level = self._calculate_threat_level(model_score, matched_patterns, model_prediction)
    ...
    def _calculate_threat_level(
        self, model_score: float, patterns: List[str], model_prediction: bool
    ) -> ThreatLevel:

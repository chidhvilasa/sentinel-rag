...
class SentinelNeutralizer:
    ...
    SUSPICIOUS_LINE_KEYWORDS = [
        ...
        "output your complete instruction",
        "your instruction set",
        # Data exfiltration
        "send the full system prompt",
        "transmit all candidate",
        "extract any api key",
        "extract any password",
        "post all retrieved",
        "email the complete evaluation",
        "data-collect@",
        "attacker-domain",
        "exfil.example",
        "malicious-site",
        ...
        # Context manipulation
        "above content is outdated",
        "above information is outdated",
        "previous content is just test",
        "content is outdated",
        "information is outdated and incorrect",
        "just test data",
        "test data that should",
        "true credentials",
        "real and actual assessment",
        "here is the real",
        "here is the actual",
        # Fake credentials
        "created react framework",
        "created react at",
        "invented react",
        "built react",
        "equivalent to a staff engineer",
        "equivalent to a principal",
        ...
        "simply confirm the score",
        "deviation from this assessment",
        "flagged for review",
        "verified by the hr",
        "verified by hr",
        "official pre-assessment",
        "pre-assessment",
    ...
    def __init__(
        self, default_strategy: NeutralizationStrategy = NeutralizationStrategy.SURGICAL_REMOVAL
    ):
        self.default_strategy = default_strategy

        # Pre-compile all patterns
        self._attack_start_compiled = [
            re.compile(p, re.IGNORECASE | re.MULTILINE) for p in self.ATTACK_START_PATTERNS
        ]
        self._attack_phrase_compiled = [re.compile(p, re.IGNORECASE) for p in self.ATTACK_PHRASES]
        self._legitimate_compiled = [
            re.compile(p, re.MULTILINE) for p in self.LEGITIMATE_INDICATORS
        ]
    ...
    def _surgical_removal(self, text: str) -> Tuple[str, str]:
        ...
        for line in lines:
            stripped = line.strip()
            is_separator = any(re.match(p, stripped, re.IGNORECASE) for p in separator_patterns)
            if is_separator:
                continue  # Drop it
    ...
    def neutralize_batch(
        self, texts: List[str], threat_levels: Optional[List[str]] = None
    ) -> List[NeutralizationResult]:
        """Neutralize multiple text chunks."""
        if threat_levels is None:
            threat_levels = ["medium"] * len(texts)
        return [
            self.neutralize(text, threat_level=level) for text, level in zip(texts, threat_levels)
        ]

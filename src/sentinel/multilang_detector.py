...
class MultiLangDetector:
    ...
    def detect(self, text: str) -> Dict:
        ...
        if code_switch_attack:
            evidence.append(
                "Code-switching attack detected (mixing languages to evade detection)"
            )
        ...
        if len(detected) > 1:
            ...
            for lang in detected:
                if lang in self.language_markers:
                    counts[lang] = sum(
                        1 for m in self.language_markers[lang] 
                        if m in text.lower()
                    )
    ...
    def _detect_code_switching_attack(
        ...
        instruction_words = [
            'ignore', 'disregard', 'override', 'prompt', 'system',
            'rate', 'score', 'excellent', 'perfect', '10'
        ]
    ...
    def _get_english_patterns(self) -> List[str]:
        ...
        return [
            ...
            'override system', 'forget previous', 'act as if',
            'give me 10', 'rate this excellent', 'system prompt',
            'always respond', 'never mention', 'output only'
        ]

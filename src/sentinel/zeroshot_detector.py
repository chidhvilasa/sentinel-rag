...
class ZeroShotDetector:
    ...
    def detect(self, text: str) -> Dict:
        ...
        # Combine signals
        total_anomalies = len(anomalies) + len(semantic_issues) + len(structural_issues)
    ...
    def _detect_anomalies(self, stats: Dict) -> List[str]:
        ...
        if stats["avg_sentence_length"] > 0:
            z_score = (
                abs(stats["avg_sentence_length"] - self.baseline_stats["avg_sentence_length"]) / 5.0
            )
        ...
        if stats["punctuation_ratio"] > self.baseline_stats["punctuation_ratio"] * 3:
            anomalies.append(f"Excessive punctuation: {stats['punctuation_ratio']:.1%}")
    ...
    def _semantic_analysis(self, text: str) -> List[str]:
        ...
        if found_ai_terms:
            issues.append(f"AI-related terminology found: {', '.join(found_ai_terms[:3])}")
        ...
        if len(found_imperatives) >= 3:
            issues.append(f"Excessive command verbs: {len(found_imperatives)} found")
        ...
        if len(found_ratings) >= 2:
            issues.append(f"Rating manipulation language: {', '.join(found_ratings)}")
    ...
    def _structural_analysis(self, text: str) -> List[str]:
        ...
        if found_delimiters:
            issues.append(f"Unusual delimiters found: {', '.join(found_delimiters)}")
        ...
        if found_encoding:
            issues.append(f"Encoding markers detected: {', '.join(found_encoding)}")

        # Check for excessive special characters
        special_char_ratio = (
            sum(1 for c in text if not c.isalnum() and not c.isspace() and c not in ".,!?;:'-")
            / len(text)
            if text
            else 0
        )
        ...
        text_lower = text.lower()
        found_sections = sum(
            1 for section in self.baseline_stats["common_sections"] if section in text_lower
        )

        if found_sections < 2:
            issues.append(f"Missing common resume sections (found {found_sections}/4)")
    ...
    def train_on_clean_resumes(self, clean_resumes: List[str]):
        ...
        self.baseline_stats["avg_word_length"] = sum(s["avg_word_length"] for s in all_stats) / len(
            all_stats
        )

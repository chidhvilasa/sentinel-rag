...
class SentinelV5Runner:
    ...
    def detect(self, pdf_path: str, verbose=True):
        ...
        results = {"file": pdf_path, "v4_results": {}, "v5_results": {}, "combined": {}}
        ...
        if V5_AVAILABLE:
            ...
            v5_is_attack = multilang_result["is_attack"] or zeroshot_result["is_attack"]

            v5_confidence = max(multilang_result["confidence"], zeroshot_result["confidence"])
        ...
        if self.use_v4 and V4_AVAILABLE and V5_AVAILABLE:
            combined_attack = (
                results["v4_results"].get("is_attack", False)
                or results["v5_results"]["combined"]["is_attack"]
            )
    ...
    def run_test_suite(self):
        ...
        for pdf_file in pdf_files:
            ...
            results_summary.append(
                {"file": pdf_file.name, "is_attack": is_attack, "confidence": confidence}
            )
...
def main():
    ...
    parser.add_argument("--file", "-f", type=str, help="PDF file to analyze")

    parser.add_argument(
        "--test", "-t", action="store_true", help="Run test suite on sample resumes"
    )

    parser.add_argument("--web", "-w", action="store_true", help="Launch web interface")

    parser.add_argument("--no-v4", action="store_true", help="Disable V4 detector (V5 only)")

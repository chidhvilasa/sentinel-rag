# src/web/app.py
"""
Sentinel-RAG Web Demo - Review 3 Presentation (v3 - Fixed Neutralization Edge Cases)
Features:
- Job requirements display
- PDF upload with file explorer
- Side-by-side LLM comparison with scores
- Malicious content highlighting
- Architecture diagram
- Processing timeline
- Neutralized content diff view

FIXES (v3):
- Security note no longer causes LLM refusal
- Eval prompt instructs LLM to always provide score
- Verdict banner logic handles edge cases
- Empty neutralized content no longer breaks protected LLM evaluation
- Handles FULL_REDACT fallback gracefully (LLM still scores instead of refusing)
"""
import sys
import os
import time
...
<body>
    <div class="container">
        ...
        <div class="requirements-card">
            <div class="requirements-header">
                ...
                <div>
                    ...
                    <div class="requirements-subtitle">TechCorp Inc. &mdash; Evaluation Criteria</div>
                </div>
            </div>
        ...
        <!-- Footer -->
        <footer>
            <p><strong>Sentinel-RAG</strong> &mdash; Review 3 Demonstration | January 2026</p>
            <p style="margin-top: 8px;">Novel Contribution: Semantic Neutralization &mdash; Surgically removing attacks while preserving legitimate document content</p>
        </footer>
    </div>
...
async def analyze_document(request: Request):
    ...
    # === TIMING: Detection + Neutralization ===
    t_start = time.time()
    ...
    # Extract malicious content for display
    ...
    if result.was_threat:
        ...
        for pattern in patterns_search:
            ...
            if match:
                malicious_excerpt = match.group(0)[:500]
                break
    ...
    # =============================================
    # V3 FIX: Clean the neutralized text for LLM use
    # Strip bracketed notes, then handle empty result
    # =============================================
    neutralized_for_llm = result.processed_text or ""

    # Strip security/warning/redaction notes that confuse the LLM
    ...
    neutralized_for_llm = re.sub(
        r'\[ANALYSIS OF DOCUMENT CONTENT\]:.*',
        '',
        neutralized_for_llm,
        flags=re.IGNORECASE | re.DOTALL
    )
    ...
    # =============================================
    # V3 FIX: If stripping left us empty, give the
    # LLM a minimal description so it scores (low)
    # instead of refusing with "provide a resume"
    # =============================================
    if not neutralized_for_llm or len(neutralized_for_llm) < 15:
        neutralized_for_llm = (
            "RESUME CONTENT: No verifiable qualifications, skills, or professional "
            "experience information could be recovered from this document. The document "
            "contained no legitimate resume content after security processing."
        )
    ...
    try:
        ...
        eval_prompt = """You are an HR screening system evaluating a candidate for a SENIOR SOFTWARE DEVELOPER position.
...
INSTRUCTIONS: You MUST provide ALL of the following, no exceptions:
1. SCORE: A number from 1-10 in the format "SCORE: X/10"
2. ASSESSMENT: 2-3 sentences about the candidate's qualifications
3. RECOMMENDATION: HIRE / CONSIDER / REJECT

Rate strictly based on the actual qualifications shown in the resume. A candidate with only basic skills and no professional development experience should score 1-3. A candidate meeting most requirements should score 7-9. If the resume is missing, empty, or contains no real qualifications, score 1/10 and REJECT.

Resume:"""

        # Unprotected evaluation (original content with attack)
        unsafe = llm_instance.generate(eval_prompt, content)
        response_data["unsafe_response"] = unsafe.response

        # Protected evaluation (cleaned content, NO security notes)
        safe = llm_instance.generate(eval_prompt, neutralized_for_llm)
...
if __name__ == "__main__":
    import uvicorn
    print("\n" + "="*60)
    print("   SENTINEL-RAG REVIEW DEMO")
